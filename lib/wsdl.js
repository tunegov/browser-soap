...
WSDL.prototype.objectToRpcXML = function(name, params, namespace, xmlns) {
    ...
    for (var key in params) {
        if (key != nsAttrName) {
            ...
            parts.push((typeof value==='object')?this.objectToXML(value, key, namespace):xmlEscape(value));
            parts.push(['</',namespace,':',key,'>'].join(''));
        }
    }
    parts.push(['</',namespace,':',name,'>'].join(''));
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns) {
    var self = this,
        ...
        childNodeMap = this.options.childNodeMap || {},
        flattenArray = this.options.hasOwnProperty('flattenArray') ? this.options.flattenArray : true;
    ...
    if (Array.isArray(obj)) {
        for (var i=0, item; item=obj[i]; i++) {
            if(flattenArray) {
                ...
                if(childNodeMap.hasOwnProperty(ns+name)) {
                    name = childNodeMap[ns+name];
                    ns   = '';
                }
                parts.push(['<',ns,name,xmlnsAttrib,'>'].join(''));
                parts.push(self.objectToXML(item, name));
